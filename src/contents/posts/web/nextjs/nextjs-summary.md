...
Next.js 14.2.7 버전 사용법을 배워보고 정리 중인 글.
...
JSX에선 for문을 사용할 수 없으므로 map() 함수를 사용하여 반복문을 구현함. 꼭 for문을 사용해야 한다면 HTML 문서 바깥 부분에서 for문을 사용한 후 결과값을 HTML에 가져다 쓰면 되긴 함.
...
**재사용이 잦은 HTML 요소들을 묶어 컴포넌트로 만드는 게 좋음.** HTML 요소들을 하나의 컴포넌트에 너무 많이 묶으면 컴포넌트가 비대해지고, 그렇다고 최대한 쪼개서 여러 가지 컴포넌트를 만들면 컴포넌트 간 데이터 공유가 복잡해질 수 있으니 적당히 쪼개고 합치는 게 좋아 보임.
...
**로컬 이미지를 넣으려면 public 폴더에 이미지를 복사**하여 사용하면 됨.
...
**모든 페이지에서 공통적으로 보여야 하는 UI는 `app/layout.js` 파일에 작성**해주면 됨(네비게이션 바 등)
...
**Props는 부모 컴포넌트가 자식 컴포넌트에 데이터(변수, 함수 등)를 전송**하는 방법.

자식 컴포넌트가 부모 컴포넌트에 Props를 전달할 수 없음. 서로 아무런 관련이 없는 컴포넌트에게도 Props를 넘겨줄 수 없음. 즉 패륜전송과 불륜전송은 불가함. 🤣
...
State의 기존 값과 새로운 값이 같으면 State 변경 함수는 값을 변경하지 않음. Array 자료형인 State 복사가 잘 안돼서 갸우뚱하는 경우가 있는데, 이 경우 Array 복사본을 만든 후 복사본을 수정하여 반영해주면 됨.
...
**Link 컴포넌트를 쓰면 페이지가 부드럽게 전환되는 링크를 만들 수 있음.**
...
**useRouter를 사용하면 다양한 라우팅 기능을 사용**할 수 있음.
...
DB를 사용하는 API 기능은 3-Tier Architecture(클라이언트 - 서버 - DB) 기반으로 만들어야 함. 클라이언트가 요청하는 대로 데이터를 DB에 마구마구 저장하면 코딩인생 끝날 수 있음... 😱 

클라이언트의 요청을 서버가 검증한 후 이를 DB에 반영하는 식으로 구현해야 함.
...
HTML Form으로 GET, POST 등 요청 시 페이지가 새로고침 됨. 반면 **Ajax 요청 시엔 페이지가 새로고침 되지 않음.**
...
**Server actions 기능을 사용하면 하나의 파일에 컴포넌트 페이지와 컴포넌트가 요청하는 동작을 처리하는 함수를 만들 수 있음.** 14 버전 이상에선 기본 지원되므로 그냥 쓰면 되고, 그 미만 버전에선 next.config.js(또는 .mjs) 파일에 아래와 같이 명시해줘야 함.
...
Server actions는 클라이언트 컴포넌트에서도 사용할 수 있음. 그런데 서버에서 해야 할 동작을 직접 사용하면 위험하므로 별도 js 파일을 만든 후 이를 import하여 사용하면 됨. (그러면 자동 라우팅 API와 차이가...? 🤔)

Server actions 사용 시 HTML 폼 전송 시 HTML 내용 변경이 안 됨. 이를 해결하려면 폼 전송 직후 `router.refresh()`를 사용하거나, 서버 컴포넌트인 경우 revalidateTag() 또는 revalidatePath()를 사용하여 HTML이 변경된 페이지 캐시를 삭제해주면 됨.
...
Next.js 프로젝트를 배포하려면 `npm run build` 명령어를 실행하여 html, css, javascript로 변환해줘야 함. 변환된 결과물은 `.next` 폴더에 저장되며, 이 결과물을 호스팅하면 됨.
...
보통 Static, Dynamic 렌더링을 알맞게 해주지면 그렇지 않은 경우도 있음. 예를 들어 DB 데이터를 보여주는 페이지는 Dynamic 렌더링 페이지가 되어야 하지만 Static 렌더링 페이지가 되는 경우가 있음. 이 경우 **page.js 코드 상단에 dynamic 예약어를 export 해서 렌더링 방식을 강제로 설정**해줘야 함.
...
Dynamic 렌더링은 서버 또는 DB의 부하를 늘릴 수 있음. 이 때 **캐싱 기능을 사용하면 부하를 줄일 수 있음.**
...
**Oauth는 특정 사이트의 사용권한을 대여하는 개념.** 소셜(구글, 애플, 깃허브 등) 로그인을 구현하고자 한다면 OAuth를 사용하면 됨.

**Next.js에서 OAuth를 사용하려면 NextAuth.js 라이브러리를 사용**하면 됨. NextAuth.js를 사용하면 소셜 로그인뿐만 아니라 ID/비밀번호 로그인 구현이 가능하며, JWT와 세션 방식도 사용할 수 있음. (단, ID/비밀번호 르그인 방식은 JWT를 강제로 사용해야 함)
