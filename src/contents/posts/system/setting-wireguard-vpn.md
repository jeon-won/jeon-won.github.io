...
## 2️⃣ ASUS 공유기 포트포워딩 설정

Advanced Settings의 WAN 메뉴 → Virtual Server / Port Forwarding 메뉴 → `Add Profile` 클릭 후 아래와 같이 추가

   * Service Name: 아무렇게나 입력
   * Protocol: UDP
   * External Port: 51820
   * Internal IP Address: ASUS 공유기 IP 입력(아마 192.168.50.1일 것임)
   * Source IP: 51820

Enable Port Forwarding을 `ON`으로 변경


## 3️⃣ 통신사 공유기 포트포워딩 설정
