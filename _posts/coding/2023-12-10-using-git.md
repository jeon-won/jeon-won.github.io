...
1. GitHub 계정 로그인 → Settings 메뉴 → Developer settings 메뉴 → Personal Access Tokens 메뉴 → Tokens (classic) 메뉴에서 토큰 생성
2. 토큰 생성 후 나타나는 토큰 값을 별도 메모
3. `git commit` 후 `git push`를 하면 GitHub Username과 Password 입력 줄이 나타남. Username에 GitHub ID를, Password에 방금 전 메모해 둔 Personal Access Token 값 입력.

```shell
$ git push
Username for 'https://github.com': <GitHub ID 입력>
Password for 'https://jeon-won@github.com': <토큰 값 입력>
```
