...
꺾쇠괄호 `>`를 사용하면 직계(바로 아래) 요소를 스타일링. 꺽쇠괄호를 연달아 쓰는 건 어떤 부분을 스타일링하는지 알기 어려우므로 권장하지 않음. 클래스를 만들어 영역을 구분한 후 스타일링 하는 게 좋음.
...
Flexbox 레이아웃을 사용하기 위한 값으로, 자식 요소들을 가로 또는 세로로 정렬함. Flexbox 레이아웃은 width 값이 정확히 맞지 않을 수 있으나 최대한 맞추려 노력은 함.
...
Grid 레이아웃을 사용하기 위한 값으로, 격자(모눈종이)처럼 요소를 배치함.
...
미디어 쿼리를 사용하면 스크린 사이즈 별 디자인을 만들 수 있음. 미디어 쿼리 문법은 맨 밑에 있어야 잘 작동함.
